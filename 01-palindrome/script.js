...
function palindrome(str) {
  const strArr = str.toLowerCase().replace(/[\s!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~]/g, '').split("");
  const strArrRevers = strArr.slice().reverse();
  return strArr.every(function (letter, n) {
    return letter == strArrRevers[n];
  });
}
